#ifndef GAME_H_INCLUDED
#define GAME_H_INCLUDED

#include <SFML/Graphics.hpp>
#include <array>
#include "game_state.h"
#include "aux_states.h"
#include "playing_state.h"

// Klasa koja predstavlja igru.
class Game{
public:
    Game();
    void run();
    void changeState(GameState::State newState){ mpCurrentGameState = mpGameStates[newState]; } //prelazak u nnovo stanje
    // ostali dio sučelja
    sf::Font font1;
    sf::Font font2;
    sf::RenderWindow mWindow;
    sf::Time mTimePerFrame;
    int mSize;
    int mScore;


private:
    void processEvents();
   // sf::RenderWindow mWindow;

    // trenutno stanje igre određeno je ovim pokazivačem.
    GameState *      mpCurrentGameState;        //ptr na trenutno stanje
    // Sva stanja igre. GameState::Count daje broj stanja. 
    std::array<GameState*,GameState::Count> mpGameStates;
    // ostale privatne varijable
    // eventualne friend deklaracije
};

#endif // GAME-V1_H_INCLUDED
