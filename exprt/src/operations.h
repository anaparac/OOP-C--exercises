...
//MATRICA & funkcijA M*v i klasA koja predstavlja množenje

template <typename T, size_t N, size_t M>
class Array2D{
private:
    //matrica tipa NxM s elementima tipa T
    T m_data[N][M]; //ako ne bude valjalo sjeti se da si stavila pod public i valjalo je
public:
...
{
    public:
        // matrica iz skalarne vrijednosti
    explicit Matrix(double val=0.0) {
        ...
        Matrix(double const polje[N][M]) {
            for(auto i=0;i<N;i++)
                for(auto j=0;j<M;j++){
                   // this->m_data[i][j]=polje[i][j];
                     this->operator ()(i,j)=polje[i][j];
                }
        }

        // pridruživanje matrica različitog tipa
        template <typename T>
